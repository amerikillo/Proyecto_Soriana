...
<jsp:useBean id="claveP" scope="session" class="modelo.Bcombo"></jsp:useBean>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>16- FORMATO CONTROL DE ANTIBIOTICOS</title>
</head>
<body onload="ocultaEtiquetas_menu16()">
	...
	<form name="frmMenu16" action="../ServletControl" onsubmit="return validarMenu16()" method="post">
		<input type="hidden" name="caso" value="160" />
		
		Clave:
		<select id="cmbClave" name="cmbClave" >
			<option value="0">-- Seleccione --</option>
			<%
				if(claveP.getDatos().size()>0){
					for(int r=0; r<claveP.getDatos().size(); r++){
						out.println("<option value = " + claveP.getDatos().get(r) + ">" + claveP.getDatos().get(r) + "</option>");
					}
				}
			%>
		</select>
		<label id="lblClave" style="color: red;">*</label>
		<br></br>
		
		Clave:
		<input id="txtId" name="txtId" value=""/>
		<br></br>
		
		Laboratorio: 
		<textarea rows="3" cols="50" id="txtLab"></textarea>
		<br></br>
		
		Descripción:
		<textarea rows="3" cols="50" id="txtDes"></textarea>
		<br></br>
		
		Presentación:
		<input id="txtPres" name="txtPres" value=""/>
		<br></br>
